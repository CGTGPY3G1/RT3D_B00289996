...
namespace rt3d {
	...
	GLuint loadBitmap(const char * fname) {
		static std::map<const char *, GLuint> map;
		GLuint texID;
		if(map.count(fname) == 0) {
	...
	void DestroyMesh(const GLuint id) {
		if(vertexArrayMap.count(id) != 0) {
			glDeleteVertexArrays(1, &id);
			vertexArrayMap.erase(id);
		}
	}
	...
	void setActiveTexture(const GLuint & textureID) {
		static GLuint tID = 0;
		if(tID != textureID) {
			tID = textureID;
